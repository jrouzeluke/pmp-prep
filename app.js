...
const PMPApp = () => {
    ...
    return (
      <div className="max-w-6xl w-full p-10 animate-fadeIn">
        <header className="mb-12 flex justify-between items-end">
            ...
            <div className="glass-card px-6 py-2 border-rose-500/20 text-rose-500 text-[10px] font-black uppercase tracking-widest">16 DAY STREAK</div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            ...
            <button key={btn.id} onClick={() => btn.id === 'hub' && setView('learning-center')}
              ...
              <span className="hud-font text-[12px] font-bold text-white text-center group-hover:scale-110 transition-transform">{btn.label}</span>
            </button>
          ))}
        </div>

        <div className="glass-card p-10 border-t-2 border-emerald-500 bg-emerald-500/5">
            <h3 className="hud-font text-xs text-emerald-400 font-black tracking-widest uppercase mb-6">Performance Dashboard</h3>
  ...
  if (view === 'learning-center') {
    const categories = {
      people: ['Manage Conflict', 'Lead a Team', 'Support Performance', 'Empower Team', 'Ensure Training', 'Build Team', 'Remove Impediments', 'Negotiate Agreements', 'Collaborate Stakeholders', 'Shared Understanding', 'Virtual Teams', 'Ground Rules', 'Mentoring', 'EQ Performance'],
      process: ['Execute Value', 'Manage Comms', 'Assess Risk', 'Engage Stakeholders', 'Budget/Resources', 'Schedule', 'Quality', 'Scope', 'Integrate Planning', 'Manage Changes', 'Procurement', 'Artifacts', 'Methodology', 'Governance', 'Issues', 'Transfer', 'Closure'],
      business: ['Compliance', 'Deliver Benefits', 'External Env', 'Org Change']
    };
  ...
  // --- VIEW 3: TASK INTERSTITIAL (CHOICE POINT) ---
  if (view === 'task-interstitial') {
    return (
      <div className="max-w-4xl w-full p-20 text-center glass-card animate-fadeIn">
        <h2 className="hud-font text-white/40 text-xs mb-4 uppercase tracking-widest">Selected Objective</h2>
        ...
        <div className="grid grid-cols-2 gap-10">
            <button onClick={() => setView('learning-designed')} className="glass-card p-16 border-cyan-500/30 hover:bg-cyan-500/10 group">
                <h3 className="hud-font text-2xl text-cyan-400 group-hover:scale-105 transition-all italic uppercase">Learn</h3>
                <p className="text-[10px] text-slate-500 mt-2 font-bold uppercase tracking-[0.3em]">Foundational mastery</p>
            </button>
            <button onClick={() => setView('practice-selection')} className="glass-card p-16 border-purple-500/30 hover:bg-purple-500/10 group">
                <h3 className="hud-font text-2xl text-purple-400 group-hover:scale-105 transition-all italic uppercase">Practice</h3>
                <p className="text-[10px] text-slate-500 mt-2 font-bold uppercase tracking-[0.3em]">Veteran Operational Modes</p>
            </button>
        </div>
        <button onClick={() => setView('learning-center')} className="mt-12 text-[10px] text-slate-600 uppercase font-black tracking-widest hover:text-white transition-colors">Cancel Selection</button>
      </div>
    );
  }

  // --- VIEW 4: LEARN (FOUNDATIONAL DEEP DIVE) ---
    ...
    return (
      <div className="max-w-6xl w-full p-12 glass-card animate-fadeIn">
        <header className="flex justify-between items-center mb-12 border-b border-white/5 pb-6">
          ...
          <h1 className="hud-font text-4xl font-black italic text-white uppercase tracking-tighter">{selectedTask}</h1>
  ...
  // --- VIEW 5: PRACTICE SELECTION (VETERAN CHOICE) ---
    ...
    return (
      <div className="max-w-5xl w-full p-16 animate-fadeIn text-center glass-card border-purple-500/30">
        <header className="mb-12">
            <button onClick={() => setView('task-interstitial')} className="text-purple-400 text-[10px] font-black tracking-widest uppercase mb-4 block hover:text-white transition-all">// BACK_TO_SELECTION</button>
            <h1 className="hud-font text-4xl italic text-white uppercase">{selectedTask}: Practice Mode</h1>
        </header>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <button onClick={() => setView('simulation')} className="glass-card p-12 border-emerald-500/30 hover:bg-emerald-500/10 group text-left">
                <h3 className="hud-font text-xl text-emerald-400 mb-4 italic group-hover:translate-x-2 transition-transform">01. Tactical Blueprint</h3>
                <p className="text-slate-400 text-[11px] leading-relaxed uppercase font-bold tracking-tight">Review the 5 Tactical Layers: Mindset, Enablers, Traps, and Artifacts.</p>
                <div className="mt-6 text-[9px] text-emerald-500/50 font-black tracking-[0.3em]">BLUEPRINT // STATIC_INTEL</div>
            </button>
            <button onClick={() => setView('decision-lab')} className="glass-card p-12 border-purple-500/30 hover:bg-purple-500/10 group text-left">
                <h3 className="hud-font text-xl text-purple-400 mb-4 italic group-hover:translate-x-2 transition-transform">02. Decision Lab</h3>
                <p className="text-slate-400 text-[11px] leading-relaxed uppercase font-bold tracking-tight">Apply tactical knowledge to a live project crisis with real-time consequences.</p>
                <div className="mt-6 text-[9px] text-purple-500/50 font-black tracking-[0.3em]">LIVE_SCENARIO // STRESS_TEST</div>
            </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="max-w-6xl w-full p-12 animate-fadeIn relative">
        <header className="flex justify-between items-center mb-10">
            ...
            <h1 className="hud-font text-4xl font-black italic text-white uppercase tracking-tighter">Practice: {selectedTask}</h1>
            <span className="hud-font text-[10px] text-purple-400 font-bold uppercase tracking-widest">Veteran Mode Active</span>
        </header>

        <div className="grid grid-cols-2 gap-8 mb-10">
          <div className="glass-card p-8 border-l-4 border-purple-500 bg-purple-500/5">
            <h4 className="hud-font text-[10px] text-purple-500 mb-4 tracking-widest">01. PMI Mindset</h4>
            <p className="text-slate-300 text-sm italic leading-relaxed">{currentTaskContent.practice.mindset}</p>
          </div>
          <div className="glass-card p-8 border-l-4 border-emerald-500 bg-emerald-500/5">
            <h4 className="hud-font text-[10px] text-emerald-500 mb-4 tracking-widest">02. Enablers</h4>
            <p className="text-slate-300 text-[10px] font-bold uppercase tracking-widest leading-loose">{currentTaskContent.practice.enablers}</p>
          </div>
          <div className="glass-card p-8 border-l-4 border-rose-500 bg-rose-500/5">
            <h4 className="hud-font text-[10px] text-rose-500 mb-4 tracking-widest">03. Precision Filters</h4>
            <p className="text-slate-300 text-[10px] font-black uppercase tracking-widest">{currentTaskContent.practice.traps}</p>
          </div>
          <div className="glass-card p-8 border-l-4 border-blue-500 bg-blue-500/5">
            <h4 className="hud-font text-[10px] text-blue-500 mb-4 tracking-widest">04. Artifacts</h4>
  ...
  // --- VIEW 7: DECISION LAB (STRESS TEST) ---
  if (view === 'decision-lab') {
    return (
      <div className="max-w-4xl w-full p-20 glass-card animate-fadeIn text-center">
         <h1 className="hud-font text-3xl italic text-purple-400 mb-6">Decision Lab Active</h1>
         <p className="text-slate-300 text-sm mb-12 uppercase font-bold tracking-[0.2em]">Live Branching Scenario Logic Loading...</p>
         <button onClick={() => setView('simulation')} className="text-[10px] text-slate-500 uppercase font-black hover:text-white transition-all tracking-widest">// ABORT SIMULATION</button>
      </div>
    );
  }
};
